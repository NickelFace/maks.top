...
5. Что происходит при наличии файла `/etc/nologin`? → Все попытки входа непривилегированных пользователей отклоняются, а содержимое файла выводится как сообщение.
...
15. В каком порядке проверяются `/etc/hosts.allow` и `/etc/hosts.deny`? → Сначала `hosts.allow`, затем `hosts.deny`. При отсутствии совпадений доступ разрешается.
