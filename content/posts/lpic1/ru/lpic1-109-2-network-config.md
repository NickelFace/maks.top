...
NetworkManager является стандартным демоном управления сетью в большинстве дистрибутивов. `nmcli` является его интерфейсом командной строки.
...
Демон `systemd-networkd` управляет сетевой конфигурацией на основе systemd.
