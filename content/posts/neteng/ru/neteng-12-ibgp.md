...
5. Настройте офис С.-Петербург так, чтобы трафик до любого офиса распределялся по двум линкам одновременно
...
### iBGP в офисе Москва между маршрутизаторами R14 и R15
...
### Настройте iBGP в провайдере Триада
...
**Резюмирую:** Ранее я приоритет local-preference выставлял на вход у R14 в сторону R15, но тогда я не решал проблемы с масштабируемостью — мне бы пришлось на каждом роутере этот параметр выставлять. В данном случае мы поступили иначе: мы все Update от R21 заранее помечаем с приоритетом 150 (neighbor 111.111.111.2 route-map LP in), и уже тогда R15 будет рассказывать об этом. ***Спасибо Алексею за консультацию***.
...
### Проверьте полную IP-связность всех сетей
