...
code_toggle: true
page_lang: "ru"
lang_pair: "/posts/neteng/neteng-12-ibgp/"
...
7. Задокументировать план работы и внесённые изменения
...
iBGP-сессия строится через Loopback0, доступный через OSPF area 0. `next-hop-self` обеспечивает доступность внешних next-hop для iBGP-пира.
...
## Москва — Ламас как приоритетный провайдер

R15 помечает все Update от R21 (Ламас) значением `local-preference 150` на входе через route-map `LP`. Это значение распространяется на всех iBGP-пиров — R14 видит LP=150 для маршрутов через Ламас и предпочитает их перед Киторн. Filter-list ограничивает анонс только локально анонсируемыми префиксами (`^$`).
...
<details>
<summary>R14 — show ip bgp (Ламас предпочтителен, LP=150)</summary>
<pre><code>
R14#show ip bgp
...
<details>
<summary>R14 — traceroute через Ламас</summary>
<pre><code>
...
> **Итог:** Выставлять local-preference на входе у R15 (вместо анонса от R14 в сторону R15) — масштабируемый подход: R15 сам распространяет LP=150 всем iBGP-пирам. *(Спасибо Алексею за консультацию.)*

---

## iBGP — Санкт-Петербург (AS 2042, без OSPF)

Для доступности loopback-адресов внутри офиса используется EIGRP. iBGP-сессии строятся между всеми четырьмя маршрутизаторами через loopback.
...
<details>
<summary>R15 / R14 ping R18 (Санкт-Петербург)</summary>
<pre><code>
...
<details>
<summary>R18 ping Лабытнанги / Чокурдах</summary>
<pre><code>
