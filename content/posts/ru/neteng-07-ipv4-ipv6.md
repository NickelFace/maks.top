...
code_toggle: true
page_lang: "ru"
lang_pair: "/posts/neteng-07-ipv4-ipv6/"
...
**Цель:** Распланировать и задокументировать адресное пространство для лабораторной топологии. Настроить IP на всех активных портах. Назначить каждый VPC в отдельный VLAN в каждом офисе. Настроить управляющие VLAN для сетевых устройств. Оптимизировать использование каналов и предотвратить широковещательные штормы. Использовать IPv4 и IPv6.
...
| Устройство | Порт | IPv4              | Примечание      | Регион |
| ---------- | ---- | ----------------- | --------------- | ------ |
...
| Устройство | Порт | IPv4             | Примечание          | IP-Loopback  | Регион |
| ---------- | ---- | ---------------- | ------------------- | ------------ | ------ |
...
| R13        | e0/0 | 172.16.1.2/24    | Агрегирующий канал  | 1.1.1.13/32  |        |
|            | e0/2 | 10.10.10.6       | R15 (local)         |              |        |
|            | e0/3 | 10.10.10.22      | R14 (local)         |              |        |
| R12        | e0/0 | 172.16.0.1/24    | Агрегирующий канал  | 1.1.1.12/32  |        |
...
|            | e0/2 | 172.16.10.1       | Агрегирующий канал  |             |                |
| R16        | e0/0 | 10.10.20.9        | R32 (local)         | 1.1.2.16    |                |
|            | e0/1 | 10.10.20.6        | R18 (local)         |             |                |
|            | e0/2 | 172.16.11.1       | Агрегирующий канал  |             |                |
...
| Устройство | Порт | IPv4         | Примечание  | VLAN | Регион     |
| ---------- | ---- | ------------ | ----------- | ---- | ---------- |
| R27        | e0/0 | 210.220.35.2 | R25 (Triada)|      | Labytnangi |

### Сводка по сетям

| Москва | Сети       | Маска | Loopback   |
| ------ | ---------- | ----- | ---------- |
...
Как устроена внутренняя адресация у провайдера — нас не касается: он предоставляет нам выход в интернет, и этого достаточно.
...
Так как Triada является провайдером для трёх наших сетей, пул адресов будет общим:
...
**Итого:**
- `2002:ACAD:0DB8::/48` — выделен провайдером Lamas
- `2CAD:1995:B0DA::/48` — выделен провайдером Triada

### VLAN

| Город      | VLAN | Назначение  |
| ---------- | ---- | ----------- |
| Msk        | 10   | Native      |
|            | 11   | Native      |
|            | 99   | Управление  |
