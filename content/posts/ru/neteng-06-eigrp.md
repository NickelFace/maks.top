...
date: 2025-10-01
description: "Лабораторная работа: базовая настройка EIGRP для IPv4 — соседи, пропускная способность, пассивные интерфейсы, таблица топологии"
tags: ["Networking", "EIGRP", "Routing", "Cisco", "OTUS"]
categories: ["Network Engineer"]
code_toggle: true
page_lang: "ru"
lang_pair: "/posts/neteng-06-eigrp/"
...
**Часть 1. Построить сеть и проверить подключение**
...
## Построить сеть и проверить подключение
...
Все соединения проверены — нет смысла показывать скриншоты, это только раздует лабораторную работу.



## Настройте маршрутизацию EIGRP
...
Потому что Router OS не даст анонсировать без неё при использовании неклассовых адресов.

Можно ли исключить маску в какой-нибудь из вышеприведённых инструкций network? Если да, то в каких?

Да — при явном анонсировании классовой сети.

## Проверьте маршрутизацию EIGRP
...
### Проанализируйте таблицу маршрутизации IP EIGRP.
...
Метрика и административная дистанция совпадают, поэтому оба пути вносятся в таблицу маршрутизации.

### Проанализируйте таблицу топологии EIGRP.
...
Почему в таблице топологии маршрутизатора R1 отсутствуют возможные преемники (feasible successor)?

Возможный преемник требует петле-свободного пути, где AD резервного маршрута меньше FD текущего преемника. Такого маршрутизатора в данной топологии не существует.

### Проверьте параметры маршрутизации EIGRP и анонсируемые сети
...
## Настройте пропускную способность и пассивные интерфейсы
...
Меняем пропускную способность на R1:
...
Посмотрим, как изменилась таблица маршрутизации на R1:
...
Теперь до сети 10.2.2.0/30 только 1 маршрут. Применяем на остальных маршрутизаторах:
...
Исходя из заданной пропускной способности, попробуйте определить, как будут выглядеть таблицы маршрутизации R2 и R3, прежде чем выполнять команду **show ip route**. Останутся ли их таблицы прежними или изменятся?

Изменится маршрут до сети 10.2.2.0/30.

### Настройте G0/0 как пассивный на R1, R2 и R3.
...
### Проверьте настройку пассивных интерфейсов.

Показываю за R1, так как за R2 и R3 аналогичная картина.
...
В данной лабораторной работе можно было ограничиться статической маршрутизацией. Каковы преимущества использования EIGRP?

- Поддерживает балансировку нагрузки по маршрутам с неравной метрикой (variance)
- Потребляет меньше ресурсов, чем OSPF
- Имеет возможный преемник (FS) для практически мгновенного переключения
- Поддерживает суммаризацию маршрутов в любой точке топологии
