...
**Используется:** все статьи `/posts/lpic2-*/`, `/kb/docs/*/`, `/about/`... (если нет своего шаблона)  
...
**Рендерится для:** любой раздел без специфичного шаблона (`/kb/docs/`, `/certs/` если нет своего)  
**Определяет блок:** `main`
