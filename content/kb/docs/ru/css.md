...
Блоки кода рендерит `themes/maks/layouts/_default/_markup/render-codeblock.html`.
Рядом лежит `render-codeblock-mermaid.html` и выигрывает для ```` ```mermaid ````: Hugo
предпочитает хук под конкретный язык общему.
...
Chroma размечает shell-builtins (`.nb`) и ключевые слова (`.k`), но никогда — голое имя
команды. Строка вроде `systemctl status postgresql` выходила из подсветчика вообще без
токенов и рисовалась плоской. `render-codeblock.html` оборачивает первое слово каждой
shell-строки в `.cmd`, который берёт `--code-cmd`.

Регулярка срабатывает только когда слово стоит вплотную к началу строкового спана —
поэтому отступ продолжения не трогается, а строка, которая и так открывается токеном
(комментарий, ключевое слово, builtin), не совпадает никогда. Ещё требуется хвостовой
пробел, таб, перевод строки или тег, чтобы `FOO=bar` не приняли за команду.

### Команда и вывод — разные объекты

Fence без языка — это **вывод**, а не «какой-то язык, который надо угадать». В `hugo.toml`
стоит `guessSyntax = false`: с включённым угадыванием Chroma помечал пустые блоки как
`fallback`, `gdscript` и хуже, раскрашивая их по чужой грамматике. Блоки вывода не
подсвечиваются вовсе — это ответ машины, поэтому они лежат на приподнятой поверхности
приглушённым `--code-out`. Отличать команду от её вывода с одного взгляда — и есть смысл;
...
| `.code-block` | Обёртка кода: строка с меткой + содержимое. Собирается на сервере в `layouts/_default/_markup/render-codeblock.html`, поэтому метка на месте ещё до запуска JS |
| `.code-block[data-kind="command"]` | Shell-блок. Утопленный колодец `--code-bg`, подсветка синтаксиса |
| `.code-block[data-kind="output"]` | Блок без языка. Приподнятая поверхность `--bg2`, линия 3px `--border2` слева, текст `--code-out`, без подсветки |
| `.code-block[data-kind="code"]` | Любой другой язык. Тот же колодец, что у `command`, без разметки `.cmd` |
| `.code-label` | Строка: язык + кнопка копирования |
| `.copy-btn` | "copy" → "ok!" (сбрасывается через 1.5с) |
| `.out-pre` | `<pre>` внутри блока вывода |
